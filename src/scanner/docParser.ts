...
export class DocParser {
  ...
  private parseFile(filePath: string): void {
    try {
      ...
      const patterns = [
        // ===== 不支持 =====
        // 不支持某个属性（修复：去掉"通过"后的空格要求）
        /不支持\s*(?:通过|调用|设置)?([a-zA-Z][a-zA-Z0-9]*)\s*(?:属性|接口|方法|API)/gi,
        /不支持\s+通过\s+([a-zA-Z][a-zA-Z0-9]*)\s+属性/gi,
        /不支持\s+(?:[\u4e00-\u9fa5]+\s+)?([a-zA-Z][a-zA-Z0-9]*)\s+属性/gi,
        /([a-zA-Z][a-zA-Z0-9]*)\s+(?:属性|接口|方法|API)\s+不支持/gi,
        // 一般性不支持
        /不支持\s+([a-zA-Z][a-zA-Z0-9]*)(?!\s+[:：])/gi,
        // 表格中的不支持
        /\|\s*([a-zA-Z][a-zA-Z0-9]*)\s*\|\s*不支持/gi,

        // ===== 不生效 =====
        /([a-zA-Z][a-zA-Z0-9]*)\s+(?:属性|接口|方法)?\s*不生效/gi,
        /(?:设置|配置|使用)?([a-zA-Z][a-zA-Z0-9]*)\s+(?:属性|接口|方法)?\s*(?:时|后)?\s*不生效/gi,
        /不生效\s*(?::|：)?\s*设置\s+([a-zA-Z][a-zA-Z0-9]*)/gi,
        /对\s+([a-zA-Z][a-zA-Z0-9]*)\s+不生效/gi,
        /([a-zA-Z][a-zA-Z0-9]*)\s+对\s+\S+\s+不生效/gi,

        // ===== 无效 =====
        /([a-zA-Z][a-zA-Z0-9]*)\s+(?:设置|配置|参数)?\s*无效/gi,
        /设置\s+([a-zA-Z][a-zA-Z0-9]*)\s+(?:无效|不起作用)/gi,
        /([a-zA-Z][a-zA-Z0-9]*)\s+参数\s+无效/gi,

        // ===== 没有效果/不起作用 =====
        /([a-zA-Z][a-zA-Z0-9]*)\s+(?:属性|接口|方法)?\s*(?:没有效果|不起作用|无法生效|不会生效)/gi,
        /使用\s+([a-zA-Z][a-zA-Z0-9]*)\s+(?:没有效果|不起作用)/gi,
        /([a-zA-Z][a-zA-Z0-9]*)\s+不会\s+(?:生效|响应|起作用)/gi,

        // ===== 不响应 =====
        /([a-zA-Z][a-zA-Z0-9]*)\s+(?:属性|接口|方法)?\s*不响应/gi,
        /不响应\s+([a-zA-Z][a-zA-Z0-9]*)/gi,

        // ===== 失效 =====
        /([a-zA-Z][a-zA-Z0-9]*)\s+(?:属性|接口|方法)?\s*失效/gi,
        /([a-zA-Z][a-zA-Z0-9]*)\s+会\s+失效/gi,

        // ===== 无法设置/不能设置 =====
        /无法\s+(?:设置|配置|修改)\s+([a-zA-Z][a-zA-Z0-9]*)/gi,
        /不能\s+(?:设置|配置|修改)\s+([a-zA-Z][a-zA-Z0-9]*)/gi,
        /([a-zA-Z][a-zA-Z0-9]*)\s+无法\s+(?:设置|配置|修改)/gi,

        // ===== 特定上下文的限制 =====
        /当\s+[^。]{0,50}?\s+([a-zA-Z][a-zA-Z0-9]*)\s+[^。]{0,30}?(?:不生效|无效|不支持|不响应)/gi,
        /(?:此|该|本)\s+[^。]{0,30}?\s+([a-zA-Z][a-zA-Z0-9]*)\s+[^。]{0,30}?(?:不生效|无效|不支持)/gi,
      ];
      ...
      const listPatterns = [
        ...
        /(?:不支持|不生效|无效)\s+(?:[\u4e00-\u9fa5]+\s+)?([a-zA-Z][a-zA-Z0-9]*(?:[、,，]\s*[a-zA-Z][a-zA-Z0-9]*)+)\s+属性/gi,
